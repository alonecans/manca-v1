...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:MANCA BOT\n'
            + 'ORG:Nurul;\n'
            + 'TEL;type=CELL;type=VOICE;waid=62838q5956151:+62 838-1595-6151\n'
            + 'END:VCARD'
...
rmenu = "HALLO KAK😊"
botinfo = "MAU SEWA, DONASI DLU YA KAK, FOLLOW IG KU AJA, KALO GA MAU DONASI"
limitt = 10000
memberLimit = 1
...
async function starts() {
	...
	dms.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonaraaa @${num.split('@')[0]} Manca-Bot Sayang Kamu:V`
				let buff = await getBuffer(ppimg)
				dms.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '*𝗠𝗔𝗔𝗙 𝗔𝗡𝗗𝗔 𝗧𝗜𝗗𝗔𝗞 𝗨𝗡𝗧𝗨𝗞 𝗠𝗘𝗡𝗚𝗚𝗨𝗡𝗔𝗞𝗔𝗡 𝗙𝗜𝗧𝗨𝗥 𝗜𝗡𝗜/𝗞𝗛𝗨𝗦𝗨𝗦 𝗢𝗪𝗡𝗘𝗥* ',
			...
			const ownerNumber = ["6283815956151@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		setTimeout( () => {
		reply('byee sayang, sampai jumpa lagi sama manca bot👋')
		}, 1100)
		setTimeout( () => {
		...
		case 'daftar':
					dms.updatePresence(from, Presence.composing)
					if (isUser) return reply('Nomer Kamu Sudah Terdaftar Di Data Base Manca Bot')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			//if (!isPublic) return reply(mess.only.publikG)
                ...
                break
                case 'bernyanyi1':
                const nyan = fs.readFileSync('./mp3/bernyanyi1.mp3')
                dms.sendMessage(from, nyan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'anjay':
                ...
                break
                case 'manca':
                const man = fs.readFileSync('./mp3/manca.mp3')
                dms.sendMessage(from, man, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'sariroti':
                const sari = fs.readFileSync('./mp3/sariroti.mp3')
                dms.sendMessage(from, sari, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'ciao':
                const ciao = fs.readFileSync('./mp3/bellsaciao.mp3')
                dms.sendMessage(from, ciao, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'susu':
                const su = fs.readFileSync('./mp3/susu.mp3')
                dms.sendMessage(from, su, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'sempai':
                const pai = fs.readFileSync('./mp3/sempai.mp3')
                dms.sendMessage(from, pai, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'kakekaku':
                const ku = fs.readFileSync('./mp3/kakekaku.mp4')
                dms.sendMessage(from, ku, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
                case 'kakakaku':
                const ka = fs.readFileSync('./mp3/kakakaku.mp4')
                dms.sendMessage(from, ka, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
				case 'gifstiker':
				case 'stiker':
				case 'sticker':
         ...
         case 'lirik':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Masalah telah di laporkan ke owner Manca Bot, laporan palsu/main2 tidak akan ditanggapi.')
                    break
                case 'request':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
                    reply('REQUEST ANDA TELAH SAMPAI ke owner Manca Bot, Requests palsu/main2 tidak akan ditanggapi.')
                    break
				case 'memeindo':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					break 
				case 'clearall':
					if (!isOwner) return reply('Mau Ngapain Kak?')
				...
				case 'kicktime':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					dms.sendMessage(from, 'Semoga Dia Tenang Di Alam Sana', text)
					}, 8000)
					setTimeout( () => {
					reply('Done Bos Ku')
					}, 7000)
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                ...
                bufferddd = await getBuffer('https://raw.githubusercontent.com/mancabot/termux-bot-wa/main/src/glitchtext.png')
                 reply(mess.wait)
                buff = await getBuffer(anu.result)
		...
		case 'happymod':
			...
			break
                ...
                if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  may = fs.readFileSync(`./mp3/manca.mp3`)
                  
                  client.sendMessage(from, may, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
                 
                  }
                  
                if (body.startsWith(`manca`)) {
                 
                  sem = fs.readFileSync(`./mp3/sempai.mp3`)
                  
                  client.sendMessage(from, sem, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
